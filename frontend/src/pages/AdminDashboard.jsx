...
const AdminDashboard = () => {
  ...
  const APPOINTMENT_COLLECTION_ID = envt_imports.appwriteCollection2Id;
  ...
  const fetchActivePatients = async (queryConditions = []) => {
    try {
      ...
      const response = await databases.listDocuments(
        DATABASE_ID,
        COLLECTION_ID,
        [...queryConditions, Query.orderDesc("AppointmentDate")]
      );
  ...
  const fetchAppointments = async () => {
    try {
      ...
      setError("Failed to load appointment records.");
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (searchRegNumber) {
        queryConditions.push(
          Query.equal("RegistrationNumber", searchRegNumber)
        );
      }
  ...
  const addNewRecord = async (newPatientData) => {
    try {
      const { $id, $databaseId, $collectionId, ...dataToInsert } =
        newPatientData;
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      ...
      if ("RemainingSessions" in dataToUpdate) {
        dataToUpdate.RemainingSessions =
          Number(dataToUpdate.RemainingSessions) || 0;
      }
  ...
  const headers = [
    ...
    { label: "Current Session", field: "RemainingSessions" },
    ...
    <div className="min-h-screen bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-200 p-6 overflow-hidden">
      <div className="max-w-full mx-auto bg-white rounded-lg shadow-xl p-6">
        ...
        <div className="mt-4 flex justify-center space-x-2">
          ...
          {[...Array(Math.ceil(patients.length / rowsPerPage)).keys()].map(
            (pageNum) => (
              <button
                key={pageNum}
